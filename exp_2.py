...
def extract_features(positions, box_length=BOX_L, rc=RC_GRAPH,
                     n_feat=8, instance_norm=False):
    """
    Unified feature extractor supporting both 5D and 8D modes.

    5D features  [r̄, σ_r, r_min, r_max, d̃]:
        Core bond-length statistics (identical to KA cross-system validation).

    8D features  [r̄, σ_r, r_min, r_max, d̃, skew, Q25, Q75]:
        Adds distribution shape (skewness proxy, quartiles) that captures
        asymmetric damage patterns invisible to the 5D set.

    Args:
        n_feat        : 5 or 8
        instance_norm : if True, apply per-graph z-score normalisation
                        (removes global magnitude, keeps relational geometry)

    Returns:
        node_feat  : (N, n_feat)  float32
    ...
    # ── 8D extension: skewness proxy, Q25, Q75 ────────────────────────────
    if n_feat == 8:
    ...
    # ── Optional instance normalisation ───────────────────────────────────
    if instance_norm:
...
def extract_5d_features(positions, box_length=BOX_L, rc=RC_GRAPH):
    return extract_features(positions, box_length, rc,
                            n_feat=8, instance_norm=INSTANCE_NORM)


def build_pyg_datasets_cfg(snapshots, n_feat=8, instance_norm=False,
                           label=None):
    """
    Parameterised dataset builder used by ablation experiments.

    Args:
        n_feat        : feature dimension (5 or 8)
        instance_norm : whether to apply per-graph z-score normalisation
        label         : short string for progress messages (e.g. "8D+norm")

    Returns:
        clf_data : list[Data]  — binary classification graphs
        reg_data : list[Data]  — regression graphs (all cycle snapshots)
    """
    ...
    for i, snap in enumerate(snapshots):
        ...
        nf, ei, ea = extract_features(pos, n_feat=n_feat,
        ...
        reg_data.append(Data(x=x_t, edge_index=ei_t, edge_attr=ea_t,
                             y=reg_label, cycle=cyc, glass_id=snap['glass_id']))
...
def run_classification(clf_data):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(labels, labels)):
        ...
        in_dim    = clf_data[0].x.shape[1]   # auto-detect 5 or 8
        model     = GATv2Classifier(in_dim=in_dim).to(DEVICE)
...
def run_regression_cfg(reg_data, max_epochs=150, patience=25,
                       label="standard"):
    """
    Regression with configurable training budget.
    Used for:
      - Standard run   : max_epochs=150, patience=25
      - Test C (long)  : max_epochs=300, patience=50
    """
    print(f"\n{'='*55}")
    print(f"  REGRESSION [{label}]  —  Predict Cycle Number")
    print(f"  {len(reg_data)} graphs | {N_FOLDS}-fold CV | "
          f"max_ep={max_epochs}, patience={patience}")
    print(f"{'='*55}")

    indices = np.arange(len(reg_data))
    kf      = KFold(n_splits=N_FOLDS, shuffle=True, random_state=42)
    fold_results = []
    in_dim = reg_data[0].x.shape[1]

    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(indices)):
        print(f"\n  ── Fold {fold_idx + 1}/{N_FOLDS} ──────────────────────")

        train_loader = DataLoader([reg_data[i] for i in train_idx],
                                  batch_size=BATCH_SIZE, shuffle=True,
                                  num_workers=2)
        val_loader   = DataLoader([reg_data[i] for i in val_idx],
                                  batch_size=BATCH_SIZE, shuffle=False,
                                  num_workers=2)

        model     = GATv2Regressor(in_dim=in_dim).to(DEVICE)
        optimizer = Adam(model.parameters(), lr=LR, weight_decay=1e-4)
        scheduler = CosineAnnealingLR(optimizer, T_max=max_epochs, eta_min=1e-5)
        scaler    = GradScaler(enabled=(DEVICE.type == 'cuda'))

        best_r2, best_mse, best_mae = -np.inf, None, None
        best_epoch   = 1
        patience_ctr = 0
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            if v_r2 > best_r2:
                best_r2, best_mse, best_mae = v_r2, v_mse, v_mae
                best_epoch   = epoch
                patience_ctr = 0
            else:
                patience_ctr += 1
        ...
        fold_results.append({'fold': fold_idx+1, 'r2': best_r2,
                              'mse': best_mse, 'mae': best_mae,
                              'epoch': best_epoch})
    ...
    print(f"\n  [{label}] MEAN  R²={np.mean(r2s):.4f}±{np.std(r2s):.4f}  "
          f"MSE={np.mean(mses):.4f}  MAE={np.mean(maes):.4f}")
    return fold_results
...
def main():
    ...
    # ────────────────────────────────────────────────────────────────────
    # Phase 1: Data Generation  (JAX — runs once, cached to disk)
    # ────────────────────────────────────────────────────────────────────
    snap_path = os.path.join(OUT_DIR, "battery_snapshots.npy")
    if os.path.exists(snap_path):
        print(f"\nLoading cached snapshots from {snap_path}")
    ...
    # ────────────────────────────────────────────────────────────────────
    # Phase 2: Structural diagnostics
    # ────────────────────────────────────────────────────────────────────
    ...
    for cyc in sorted(set(s['cycle'] for s in snapshots)):
        all_bonds = []
        for pos in [s['positions'] for s in snapshots if s['cycle'] == cyc][:20]:
            dr   = pos[:, None, :] - pos[None, :, :]
            dr   = dr - BOX_L * np.round(dr / BOX_L)
            dist = np.sqrt(np.sum(dr**2, axis=-1))
            mask = (dist > 1e-6) & (dist < RC_GRAPH)
    ...
    plot_cycle_distribution(snapshots,
    ...
    fat_pos   = next((s['positions'] for s in snapshots
                      if s['cycle'] == N_CYCLES and s['glass_id'] == 0), None)
    if fat_pos is not None:
        plot_force_chains(prist_pos, fat_pos,
                          os.path.join(OUT_DIR, "force_chains_battery.png"))
    gc.collect()

    # ════════════════════════════════════════════════════════════════════
    # BASELINE  —  8D features, no instance normalisation
    # ════════════════════════════════════════════════════════════════════
    print("\n" + "═"*55)
    print("  BASELINE: 8D features, INSTANCE_NORM=False")
    print("═"*55)
    clf_base, reg_base = build_pyg_datasets_cfg(
        snapshots, n_feat=8, instance_norm=False, label="8D, no-norm")
    base_clf = run_classification(clf_base)
    plot_learning_curves(
        base_clf[1], base_clf[2],
        title="Baseline: 8D features, no instance norm (5-fold CV)",
        out_path=os.path.join(OUT_DIR, "baseline_clf_curves.png"))
    base_reg = run_regression_cfg(reg_base, max_epochs=MAX_EPOCHS,
                                  patience=PATIENCE, label="baseline")
    gc.collect()

    # ════════════════════════════════════════════════════════════════════
    # TEST A  —  8D features + instance normalisation
    #            Tests whether signal survives magnitude removal.
    #            Mirrors Experiment 6 from the thermal history paper.
    # ════════════════════════════════════════════════════════════════════
    print("\n" + "═"*55)
    print("  TEST A: 8D features + INSTANCE_NORM=True")
    print("  (mirrors thermal-paper Experiment 6 / blindfold test)")
    print("═"*55)
    clf_A, reg_A = build_pyg_datasets_cfg(
        snapshots, n_feat=8, instance_norm=True, label="8D, norm")
    res_A_clf = run_classification(clf_A)
    plot_learning_curves(
        res_A_clf[1], res_A_clf[2],
        title="Test A: 8D features + instance norm (5-fold CV)",
        out_path=os.path.join(OUT_DIR, "testA_clf_curves.png"))
    res_A_reg = run_regression_cfg(reg_A, max_epochs=MAX_EPOCHS,
                                   patience=PATIENCE, label="Test-A")
    gc.collect()

    # ════════════════════════════════════════════════════════════════════
    # TEST B  —  5D features, no instance normalisation
    #            Ablates the skewness/Q25/Q75 features to quantify
    #            how much distribution shape contributes vs just 5D.
    # ════════════════════════════════════════════════════════════════════
    print("\n" + "═"*55)
    print("  TEST B: 5D features (ablate skewness/Q25/Q75),")
    print("          INSTANCE_NORM=False")
    print("═"*55)
    clf_B, reg_B = build_pyg_datasets_cfg(
        snapshots, n_feat=5, instance_norm=False, label="5D, no-norm")
    res_B_clf = run_classification(clf_B)
    plot_learning_curves(
        res_B_clf[1], res_B_clf[2],
        title="Test B: 5D features only (ablate shape features) (5-fold CV)",
        out_path=os.path.join(OUT_DIR, "testB_clf_curves.png"))
    res_B_reg = run_regression_cfg(reg_B, max_epochs=MAX_EPOCHS,
                                   patience=PATIENCE, label="Test-B")
    gc.collect()

    # ════════════════════════════════════════════════════════════════════
    # TEST C  —  8D features, long regression training
    #            Fold 1 baseline got R²=0.32 at epoch 136 before early stop.
    #            Extended budget (300 ep, patience 50) tests whether the
    #            other folds also contain learnable cycle-number signal.
    # ════════════════════════════════════════════════════════════════════
    print("\n" + "═"*55)
    print("  TEST C: 8D features, extended regression")
    print("          (max_epochs=300, patience=50)")
    print("═"*55)
    # Reuse clf_base graphs — only regression training changes
    res_C_reg = run_regression_cfg(reg_base, max_epochs=300,
                                   patience=50, label="Test-C (long)")
    gc.collect()
    ...
    print("─"*70)
    print(f"  {'Experiment':<28} {'Clf Acc (%)':<18} {'Clf AUC':<12} "
          f"{'Reg R²'}")
    print("─"*70)
    ...
    rows = [
        ("Baseline (8D, no norm)",
         base_clf[0], base_reg),
        ("Test A: 8D + inst-norm",
         res_A_clf[0], res_A_reg),
        ("Test B: 5D only",
         res_B_clf[0], res_B_reg),
        ("Test C: 8D, long reg",
         base_clf[0], res_C_reg),   # same clf as baseline
    ]

    for name, clf_res, reg_res in rows:
        acc_m, acc_s = _mean_std(clf_res, "best_acc")
        auc_m, auc_s = _mean_std(clf_res, "best_auc")
        r2_m,  r2_s  = _mean_std(reg_res, "r2")
        print(f"  {name:<28} {acc_m:>6.2f} ± {acc_s:<6.2f}   "
              f"{auc_m:.4f}      {r2_m:>+.4f} ± {r2_s:.4f}")

    print("─"*70)
    print("  Interpretation guide:")
    print("  · Baseline vs Test A  → does signal survive magnitude removal?")
    print("  · Baseline vs Test B  → how much do skew/Q25/Q75 contribute?")
    print("  · Baseline vs Test C  → is cycle-regression stable with more training?")
    print("═"*70)
